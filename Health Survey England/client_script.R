...
# weigh the numerical scores using the interview weights
hse_2000 <- hse_2000 %>% mutate_at(vars(start_col:end_col), list(~ . * Int_Weight))
...
# weigh the numerical scores using the interview weights
hse_2001 <- hse_2001 %>% mutate_at(vars(start_col:end_col), list(~ . * Int_Weight))
...
hse_2002 <- hse_2002 %>% mutate_at(vars(start_col:end_col), list(~ . * Int_Weight))
...
hse_2003 <- hse_2003 %>%
  mutate(across(all_of(cols), ~case_when(. == 1 ~ 0,
                                         ...
                                         . == 4 ~ 3),
                .names = "{.col}")) %>%
  mutate(across(ghqconc:anxiety, ~ . * int_wt))
...
#retain complete cases//// 1.6k observations removed
hse_2003_complete <- hse_2003 %>% rename(GHQg2 = ghqg2, Int_Weight = int_wt, Age = age, GHQ36Scr = ghq36scr, Weight = weight) %>%
  mutate(Age = cut(Age, breakpoints)) %>%
  mutate(Age = factor(Age, labels = c("16-24","25-34", "35-44", "45-54","55-64"))) %>%
  select(-sex)
...
hse_2004 <- hse_2004 %>%
  mutate(across(all_of(cols), ~case_when(. == 1 ~ 0,
                                         ...
                                         . == 4 ~ 3),
                .names = "{.col}")) %>%
  mutate(across(ghqconc:ghqhappy, ~ . * wt_int))
...
hse_2005 <- hse_2005_raw %>%
  ...
  select(year, pserial, age, sex, weight, wt_int, econact, topqual3, GHQg2, genhelf2, 
         ghqconc, ghqdecis, ghqsleep, ghqconfi, ghquse, ghqstrai, ghqover, ghqenjoy, ghqface, ghqunhap, ghqworth, ghqhappy, mobility, selfcare, usualact, pain, anxiety) %>%
  mutate(across(ghqconc:anxiety, ~ . * wt_int))
...
hse_2007 <- hse_2007_raw %>%
  ...
  mutate(econact = fct_recode(as.factor(activb), `In employment` = '2', `Unemployment` = '3', `Unemployment` ='4',
                              ...
                              `Other economically inactive` = '1', `Retired` ='9', `Other economically inactive` ='95', `Other economically inactive` ='10'), 
         year = "2007") %>%
  select(year, pserial, age, sex, weight, wt_int, econact, topqual3, genhelf2) #NO VARIABLE TO WEIGH FOR 2007
...
hse_2011 <- hse_2011_raw %>%
  ...
  mutate(across(Mobility:Anxiety, ~ . * wt_int))
...
hse_2013 <- hse_2013_raw %>%
  ...
  mutate(econact = fct_recode(as.factor(Activb), `In employment` = '2', `Unemployment` = '3', `Unemployment` ='4',
         ...
         year = "2013") %>%
  select(year, pserial, Age, Sex, Weight, wt_int, econact, topqual3, genhelf2) #NOTHING TO WEIGH
...
hse_2015 <- hse_2015_raw %>%
  ...
  select(year, SerialA, Age, Sex, Weight, wt_int, topqual3, genhelf2, econact) #NOTHING TO WEIGH
...
hse_2017 <- hse_2017_raw %>%
  ...
  mutate(across(Mobil17:Anxiet17, ~ . * wt_int))
...
hse_2018 <- hse_2018_raw %>%
  ...
  mutate(across(GHQCONC:Anxiet17, ~ . * wt_int))
...
hse_2019 <- hse_2019_raw %>%
  ...
  mutate(econact = fct_recode(as.factor(Activb2), `In employment` = '2', `Unemployment` = '3', `Unemployment` ='4',
         ...
         year = "2019",
         Age = fct_recode(as.factor(ag16g10), `16-24` = '1', `25-34` = '2', `35-44` = '3', `45-54` = '4', `55-64` = '5')) %>%
  select(year, SerialA, Age, Sex, Weight, wt_int, econact, topqual3, GenHelf2) #NOTHING TO WEIGH
